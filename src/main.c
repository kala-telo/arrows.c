...
#define ZOOM_FACTOR 0.02f

#define DEFAULT_CAMERA_POS (Vector2){0, 0}
...
// returned value is always not NULL
chunk_t* map_get_chunk(map_t *map, int32_t x, int32_t y) {
...
enum INPUT_EVENT {
    ...
    ARROW_DIR_FLIP,
    AREA_SELECT,
    AREA_SELECTED_REMOVE,
    AREA_SELECTED_COPY,
    AREA_SELECTED_COPY_TEXT,
    AREA_SELECTED_CUT,
    AREA_COPIED_PASTE,
    AREA_COPIED_PASTE_TEXT,
    UNDO,
    REDO,
    HOTBAR_ARROW1_SELECT,
    HOTBAR_ARROW2_SELECT,
    HOTBAR_ARROW3_SELECT,
    HOTBAR_ARROW4_SELECT,
    HOTBAR_ARROW5_SELECT,
    HOTBAR_ARROW_DESELECT,
    HOTBAR_GROUP_PREVIOUS,
    HOTBAR_GROUP_NEXT,
    INVENTORY_TOGGLE,
    PIPETTE_USE,
    CAMERA_MOVE_MOUSE,
...
KeyboardKey key_bindings[] = {
    [ARROW_REMOVE] = KEY_R,
    [ARROW_DIR_SET_N] = KEY_W,
    [ARROW_DIR_SET_S] = KEY_S,
    [ARROW_DIR_SET_W] = KEY_A,
    [ARROW_DIR_SET_E] = KEY_D,
    [ARROW_DIR_FLIP] = KEY_F,
    [AREA_SELECT] = KEY_E,
    [AREA_SELECTED_REMOVE] = KEY_BACKSPACE,
    [AREA_SELECTED_COPY] = KEY_C,
    [AREA_SELECTED_COPY_TEXT] = KEY_C, // TODO: somehow bind it on KEY_LEFT_SHIFT+KEY_C(for the Shift to be non-hardcoded)
    [AREA_SELECTED_CUT] = KEY_X,
    [AREA_COPIED_PASTE] = KEY_V,
    [AREA_COPIED_PASTE_TEXT] = KEY_V, // TODO: somehow bind it on KEY_LEFT_SHIFT+KEY_V(for the Shift to be non-hardcoded)
    [UNDO] = KEY_Z,
    [REDO] = KEY_Y,  // TODO: somehow also bind redo on KEY_LEFT_SHIFT+KEY_Z(non-hardcoded)
    [HOTBAR_ARROW1_SELECT] = KEY_ONE,
    [HOTBAR_ARROW2_SELECT] = KEY_TWO,
    [HOTBAR_ARROW3_SELECT] = KEY_THREE,
    [HOTBAR_ARROW4_SELECT] = KEY_FOUR,
    [HOTBAR_ARROW5_SELECT] = KEY_FIVE,
    [HOTBAR_ARROW_DESELECT] = KEY_GRAVE,
    [HOTBAR_GROUP_PREVIOUS] = KEY_T, // TODO: remove this bind when hotbar GUI will be done
    [HOTBAR_GROUP_NEXT] = KEY_G, // TODO: remove this bind when hotbar GUI will be done
    [INVENTORY_TOGGLE] = KEY_TAB,
    [PIPETTE_USE] = KEY_Q,
    [CAMERA_MOVE_FORWARD] = KEY_UP,
    [CAMERA_MOVE_BACK] = KEY_DOWN,
    [CAMERA_MOVE_LEFT] = KEY_LEFT,
    [CAMERA_MOVE_RIGHT] = KEY_RIGHT,
    [CAMERA_POS_SET_MAP_CENTER] = KEY_H,
    [CAMERA_ZOOM_IN] = KEY_EQUAL, // TODO: somehow also bind it on KEY_KP_ADD(non-hardcoded)
    [CAMERA_ZOOM_OUT] = KEY_MINUS, // TODO: somehow also bind it on KEY_KP_SUBTRACT(non-hardcoded)
    [CAMERA_ZOOM_RESET] = KEY_EQUAL, // TODO: somehow bind it on KEY_LEFT_SHIFT+KEY_EQUAL(for the Shift to be non-hardcoded) and somehow also bind it on KEY_LEFT_SHIFT+KEY_KP_ADD(non-hardcoded)
    [MAP_SIGNALS_REMOVE] = KEY_N,
    [GAME_PAUSE_TOGGLE] = KEY_SPACE,
    [MAP_MENU_TOGGLE] = KEY_ESCAPE,
};
MouseButton mouse_bindings[] = {
    [ARROW_SET] = MOUSE_BUTTON_LEFT,
    [ARROW_INTERACT] = MOUSE_BUTTON_LEFT,
    [CAMERA_MOVE_MOUSE] = MOUSE_BUTTON_MIDDLE, // TODO: somehow also bind camera movement on MOUSE_BUTTON_LEFT(non-hardcoded)
};

enum CellType groups[5][5] = {
    {Arrow, Source, Blocker, Delay, Detector},
    {SplitterUpDown, SplitterUpRight, SplitterUpRightLeft, Pulse},
    {BlueArrow, Diagonal, BlueSplitterUpUp, BlueSplitterRightUp, BlueSplitterUpDiagonal},
    {Not, And, Xor, Latch, Flipflop},
    {Random, Button, DirectoinalButton},
};
...
typedef struct Settings {
    ...
    float tps;
    Vector2 camera, last_mouse_pos;
    float zoom;
    bool pause;
    map_t *map;
    arrow_t *hb_arrow;
    int group;
    Vector2 selected_area[];
} settings_t;

void zoom_change(float factor, settings_t *settings) {
    Vector2 scr_size = (Vector2){GetScreenWidth(), GetScreenHeight()};
    float prev_zoom = settings->zoom;

    settings->zoom *= factor;

    settings->camera.x += (scr_size.x / prev_zoom - scr_size.x / settings->zoom) * ((settings->last_mouse_pos.x - scr_size.x / 2) / scr_size.x);
    settings->camera.y += (scr_size.y / prev_zoom - scr_size.y / settings->zoom) * ((settings->last_mouse_pos.y - scr_size.y / 2) / scr_size.y);
}

void arrow_change(arrow_t *arrow, enum CellType type, enum Direction dir, bool flipped) {
    arrow->type = type;
    arrow->direction = dir;
...
void handle_input(settings_t *settings) {
    Vector2 mouse_arrow_pos = (Vector2){
        round(settings->camera.x + (settings->last_mouse_pos.x - GetScreenWidth() / 2) / settings->zoom),
        round(settings->camera.y + (settings->last_mouse_pos.y - GetScreenHeight() / 2) / settings->zoom)
    };
    arrow_t *mouse_arrow = map_get(settings->map, mouse_arrow_pos.x, mouse_arrow_pos.y);

    if (IsMouseButtonDown(mouse_bindings[ARROW_SET])) { // FIXME: arrow sets when interacting with gui
        if (settings->hb_arrow->type != Empty) {
            arrow_change(mouse_arrow, settings->hb_arrow->type, settings->hb_arrow->direction, settings->hb_arrow->flipped);
        }

        map_get_chunk(settings->map, mouse_arrow_pos.x, mouse_arrow_pos.y)->unload_timer = 0;
    }
    if (IsKeyDown(key_bindings[ARROW_REMOVE])) {
        arrow_change(mouse_arrow, Empty, D_NORTH, false);
    }
    if (IsMouseButtonDown(mouse_bindings[ARROW_INTERACT])) { if (settings->hb_arrow->type == Empty) {} } // TODO: interacting with buttons
    if (IsKeyDown(key_bindings[ARROW_DIR_SET_N])) {
        if (settings->hb_arrow->type != Empty) settings->hb_arrow->direction = D_NORTH;
        else mouse_arrow->direction = D_NORTH;
    }
    if (IsKeyDown(key_bindings[ARROW_DIR_SET_S])) {
        if (settings->hb_arrow->type != Empty) settings->hb_arrow->direction = D_SOUTH;
        else mouse_arrow->direction = D_SOUTH;
    }
    if (IsKeyDown(key_bindings[ARROW_DIR_SET_W])) {
        if (settings->hb_arrow->type != Empty) settings->hb_arrow->direction = D_WEST;
        else mouse_arrow->direction = D_WEST;
    }
    if (IsKeyDown(key_bindings[ARROW_DIR_SET_E])) {
        if (settings->hb_arrow->type != Empty) settings->hb_arrow->direction = D_EAST;
        else mouse_arrow->direction = D_EAST;
    }
    if (IsKeyPressed(key_bindings[ARROW_DIR_FLIP])) {
        if (settings->hb_arrow->type != Empty) settings->hb_arrow->flipped = !settings->hb_arrow->flipped;
        else mouse_arrow->flipped = !mouse_arrow->flipped;
    }
    if (IsKeyPressed(key_bindings[AREA_SELECT])) {
        settings->selected_area[0] = mouse_arrow_pos;
    }
    if (IsKeyReleased(key_bindings[AREA_SELECT])) {
        settings->selected_area[1] = mouse_arrow_pos;
    }
    // TODO: interacting with selected area 
    if (IsKeyPressed(key_bindings[AREA_SELECTED_REMOVE])) {}
    if (IsKeyPressed(!IsKeyDown(KEY_LEFT_SHIFT) && key_bindings[AREA_SELECTED_COPY])) {}
    if (IsKeyPressed(IsKeyDown(KEY_LEFT_SHIFT) && key_bindings[AREA_SELECTED_COPY_TEXT])) {}
    if (IsKeyPressed(key_bindings[AREA_SELECTED_CUT])) {}
    if (IsKeyPressed(!IsKeyDown(KEY_LEFT_SHIFT) && key_bindings[AREA_COPIED_PASTE])) {}
    if (IsKeyPressed(IsKeyDown(KEY_LEFT_SHIFT) && key_bindings[AREA_COPIED_PASTE_TEXT])) {}
    if (IsKeyPressed(key_bindings[UNDO])) {}
    if (IsKeyPressed(key_bindings[REDO]) || (IsKeyDown(KEY_LEFT_SHIFT) && IsKeyDown(KEY_Z))) {}
    if (IsKeyPressed(key_bindings[HOTBAR_ARROW1_SELECT])) {
        settings->hb_arrow->type = groups[settings->group][0];
    }
    if (IsKeyPressed(key_bindings[HOTBAR_ARROW2_SELECT])) {
        settings->hb_arrow->type = groups[settings->group][1];
    }
    if (IsKeyPressed(key_bindings[HOTBAR_ARROW3_SELECT])) {
        settings->hb_arrow->type = groups[settings->group][2];
    }
    if (IsKeyPressed(key_bindings[HOTBAR_ARROW4_SELECT])) {
        settings->hb_arrow->type = groups[settings->group][3];
    }
    if (IsKeyPressed(key_bindings[HOTBAR_ARROW5_SELECT])) {
        settings->hb_arrow->type = groups[settings->group][4];
    }
    if (IsKeyPressed(key_bindings[HOTBAR_ARROW_DESELECT])) {
        settings->hb_arrow->type = Empty;
    }
    if (IsKeyPressed(key_bindings[HOTBAR_GROUP_PREVIOUS])) {
        settings->group = (settings->group - 1) < 0? 4 : (settings->group - 1);
    }
    if (IsKeyPressed(key_bindings[HOTBAR_GROUP_NEXT])) {
        settings->group = (settings->group + 1) % 5;
    }
    if (IsKeyPressed(key_bindings[INVENTORY_TOGGLE])) {} // TODO: inventory with arrows GUI
    if (IsKeyDown(key_bindings[PIPETTE_USE])) {
        if (mouse_arrow->type == Empty) settings->hb_arrow->type = Empty;
        else arrow_change(settings->hb_arrow, mouse_arrow->type, mouse_arrow->direction, mouse_arrow->flipped);
    }
    if (IsMouseButtonDown(mouse_bindings[CAMERA_MOVE_MOUSE]) || (IsMouseButtonDown(MOUSE_BUTTON_LEFT) && settings->hb_arrow->type == Empty)) {
        settings->camera.x += (settings->last_mouse_pos.x - GetMousePosition().x) / settings->zoom;
        settings->camera.y += (settings->last_mouse_pos.y - GetMousePosition().y) / settings->zoom;
    }
    if (IsKeyDown(key_bindings[CAMERA_MOVE_FORWARD])) {
        settings->camera.y -= CAMERA_SPEED * (DEFAULT_ZOOM / settings->zoom);
    }
    if (IsKeyDown(key_bindings[CAMERA_MOVE_BACK])) {
        settings->camera.y += CAMERA_SPEED * (DEFAULT_ZOOM / settings->zoom);
    }
    if (IsKeyDown(key_bindings[CAMERA_MOVE_LEFT])) {
        settings->camera.x -= CAMERA_SPEED * (DEFAULT_ZOOM / settings->zoom);
    }
    if (IsKeyDown(key_bindings[CAMERA_MOVE_RIGHT])) {
        settings->camera.x += CAMERA_SPEED * (DEFAULT_ZOOM / settings->zoom);
    }
    if (IsKeyPressed(key_bindings[CAMERA_POS_SET_MAP_CENTER])) {
        settings->camera = DEFAULT_CAMERA_POS;
    }
    if (GetMouseWheelMove() != 0) {
        zoom_change(1 + ZOOM_FACTOR * 4 * GetMouseWheelMove(), settings);
    }
    if ((!IsKeyDown(KEY_LEFT_SHIFT) && (IsKeyDown(key_bindings[CAMERA_ZOOM_IN]) || IsKeyDown(KEY_KP_ADD)))) {
        zoom_change(1 + ZOOM_FACTOR, settings);
    }
    if (IsKeyDown(key_bindings[CAMERA_ZOOM_OUT]) || IsKeyDown(KEY_KP_SUBTRACT)) {
        zoom_change(1 - ZOOM_FACTOR, settings);
    }
    if ((IsKeyDown(KEY_LEFT_SHIFT) && (IsKeyPressed(key_bindings[CAMERA_ZOOM_RESET]) || IsKeyPressed(KEY_KP_ADD)))) {
        zoom_change(DEFAULT_ZOOM / settings->zoom, settings);
    }
    if (IsKeyPressed(key_bindings[MAP_SIGNALS_REMOVE])) {} // TODO: remove all the signals on map
    if (IsKeyPressed(key_bindings[GAME_PAUSE_TOGGLE])) {
        settings->pause = 1 - settings->pause;
    }
    if (IsKeyPressed(key_bindings[MAP_MENU_TOGGLE])) {} // TODO: map menu GUI

    settings->last_mouse_pos = GetMousePosition();
}
...
int main(int argc, char** argv) {
    ...
    settings.map = &map;
    settings.hb_arrow = &(arrow_t){BlueSplitterRightUp, 0, S_NONE, D_NORTH, false};
    settings.group = 0;
        ...
        for(size_t i = 0; i < hmlen(map.chunks); i++) {
            ...
            for(size_t j = 0; j < CHUNK_SIZE*CHUNK_SIZE; j++) {
                ...
                if (arrow->signal != S_NONE) {
                    DrawRectangleV((Vector2){dest_rect.x - settings.zoom/2, dest_rect.y - settings.zoom/2}, (Vector2){settings.zoom, settings.zoom}, ({
